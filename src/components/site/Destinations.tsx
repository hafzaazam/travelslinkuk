    ...
    <section className="bg-gradient-soft py-24 px-5 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {DEST.map((d) => {
            const className = "group relative h-72 overflow-hidden rounded-3xl shadow-card hover:shadow-glow transition";
            const inner = (
              <>
            ...
            if (d.slug) {
              return (
                <Link key={d.name} to="/countries/$slug" params={{ slug: d.slug }} className={className}>
                  {inner}
                </Link>
              );
            }
            return (
              <Link key={d.name} to="/countries" className={className}>
                {inner}
              </Link>
            );
          })}
